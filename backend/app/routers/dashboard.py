...
All numbers are computed live from PostgreSQL on every request.
"""
...
def _row_from_assessment(db: Session, shipment, assessment: dict) -> dict:
    ...
    try:
        alerts = crud.get_alerts_for_shipment(db, shipment.id, skip=0, limit=1000)
        active_alerts = sum(1 for a in alerts if not getattr(a, "acknowledged", False))
    except Exception:  # noqa: BLE001
        active_alerts = 0
    readings = int((metrics.get("total_readings") or 0) or assessment.get("readings_used") or 0)
    return {
        ...
        "cumulative_excursion_minutes": metrics.get("total_excursion_minutes"),
        "remaining_shelf_life_hours": metrics.get("remaining_shelf_life_hours"),
...
def _assess_all(db: Session, limit: int):
    shipments = crud.get_shipments(db, skip=0, limit=limit)
    rows = []
    for shipment in shipments:
        product = crud.get_product(db, shipment.product_id)
        try:
            assessment = risk_service.assess_shipment(
                db, shipment, product=product, include_shap=False, use_cache=True
            )
        except Exception as exc:  # noqa: BLE001 - one bad row must not break board
            assessment = {
                "risk_level": "UNKNOWN",
                "risk_score": None,
        ...
        rows.append(_row_from_assessment(db, shipment, assessment))
    return shipments, rows


@router.get("/summary")
def dashboard_summary(db: Session = Depends(get_db)):
    """Summary cards computed live from PostgreSQL (nothing hard-coded)."""
    limit = int(settings.dashboard_max_shipments)
    shipments, rows = _assess_all(db, limit)
    breakdown = {"LOW": 0, "MEDIUM": 0, "HIGH": 0, "CRITICAL": 0, "UNKNOWN": 0}
    for row in rows:
        level = row.get("risk_level", "UNKNOWN")
        breakdown[level] = breakdown.get(level, 0) + 1
    active = [s for s in shipments if str(s.status).lower() in ACTIVE_STATUSES]
    try:
        active_alerts = crud.count_alerts(db, acknowledged=False)
    except Exception:  # noqa: BLE001
        active_alerts = 0
    ml = ml_inference.registry.summary()
    assessed = sum(1 for r in rows if r.get("risk_level") != "UNKNOWN")
    notes = [
    ...
    if len(shipments) >= limit:
        notes.append(f"Capped at {limit} shipments (DASHBOARD_MAX_SHIPMENTS).")
    return {
        "total_shipments": len(shipments),
        "active_shipments": len(active),
        "high_or_critical_risk": breakdown.get("HIGH", 0) + breakdown.get("CRITICAL", 0),
        "active_alerts": active_alerts,
        "risk_breakdown": breakdown,
        "active_statuses": sorted({str(s.status) for s in active}),
...
def dashboard_shipments(
    ...
    cap = min(int(limit), int(settings.dashboard_max_shipments))
    _, rows = _assess_all(db, cap)
    return rows


@router.get("/map")
def dashboard_map(db: Session = Depends(get_db)):
    """Live map payload: assessed shipments with coordinates where available."""
    _, rows = _assess_all(db, int(settings.dashboard_max_shipments))
    with_coords = [r for r in rows if r.get("latitude") is not None]
    latest_ts = None
    for row in rows:
